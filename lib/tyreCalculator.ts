import { TyreDimension, TyreSpecs, UnitSystem } from '../types/tyre';
 ...
 */
export function calculateSidewallHeight(width: number, aspectRatio: number): number {
 ...
 */
export function calculateRimDiameterMm(rimInches: number): number {
 ...
 */
export function calculateOverallDiameter(rimMm: number, sidewallMm: number): number {
  return rimMm + (2 * sidewallMm);
}

/**
 * Calculates rolling circumference in millimetres.
 * Formula: π × overall diameter in mm
 */
export function calculateCircumference(overallDiameterMm: number): number {
 ...
 */
export function calculateRevsPerKm(circumferenceMm: number): number {
 ...
 */
export function calculateTyreSpecs(input: TyreDimension): TyreSpecs {
 ...
 */
export function calculateDiameterDiffPct(newDiameter: number, originalDiameter: number): number {
 ...
 */
export function calculateSpeedometer100(newDiameter: number, originalDiameter: number): number {
  if (!originalDiameter || originalDiameter <= 0) return 100;
  return Number((100 * (newDiameter / originalDiameter)).toFixed(1));
}

/**
 * Calculates ground clearance change in mm (positive = higher ground clearance, negative = lower).
 */
export function calculateClearanceChangeMm(newDiameter: number, originalDiameter: number): number {
  if (!originalDiameter || originalDiameter <= 0) return 0;
...
export function formatWidth(widthMm: number, unit: UnitSystem): string {
  if (unit === 'IN') {
...
export function formatLength(lengthMm: number, unit: UnitSystem): string {
  if (unit === 'IN') {
