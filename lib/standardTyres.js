  ...
  for (let r = minRim; r <= maxRim; r++) {
    for (const w of STANDARD_WIDTHS) {
      for (const ar of STANDARD_ASPECT_RATIOS) {
        ...
        // Include candidate if within extended target window (up to ±3.0% dia & ±15% width) or if OEM approved
        if (screening.isWithinExtended || screening.isManufacturerApproved) {
